...
use std::{mem::size_of, sync::{Mutex,atomic}, time::{Duration, SystemTime, UNIX_EPOCH}};
...
pub async fn issue<'a>(state:& TokenState<'a>,data:TokenData)->String{
    ...
    state.cache.lock().unwrap().put((count,seed),(now+TTL,data));
