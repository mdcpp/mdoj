...
pub fn InnerProblemContent(
    ...
    view! {
        <div class=tw_join!("p-4 rounded size-full flex flex-col", class)>
            <h1 class="text-2xl my-2">{full_info.info.title}</h1>

            <div class="grow relative overflow-y-auto bg-black-900">
                <Markdown content=full_info.content class="absolute h-full w-full top-0 left-0" />
            </div>

            <hr class="border-t-2 border-black-400 mx-1" />

            <ul class="flex flex-row justify-center items-center space-x-4 p-1">
                <li>Memory : {full_info.memory >> 20}MB</li>
                <div class="h-auto border-l-2 border-black-400"></div>
                <li>Time : {full_info.time / 1000}MS</li>
                <div class="h-auto border-l-2 border-black-400"></div>
                <li class="flex flex-row flex-nowrap items-center gap-x-4">
                    <p>Difficulty :</p>
                    <DifficultyBadge difficulty=full_info.difficulty />
                </li>
            </ul>
        </div>
    }
